        ...
        def preprocess(graph: nx.DiGraph) -> nx.DiGraph:
            ...
            for n in graph.nodes:
                ...
                new_node: SubQuery = SubQuery(
                    old_node.query,
                    old_node.query_raw,
                    f"{old_node.alias}{hash(old_node)}",
                )
                print(old_node.query_raw, hash(old_node))
                for successor in graph.successors(old_node):
                    ...
                    if succ_old_node.parent is None:
                        raise Exception(f"Column {succ_old_node!r} without parent")
                        return graph
